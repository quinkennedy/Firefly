		...
		maingame.initializeGame=function() {
			...
			// An object will draw the maze on the screen
			 gbox.addObject({
				...
				maze=help.finalizeTilemap({tileset:"tiles",map:arrMap,tileIsSolid:function(){return false;}});
			 ...
			 if (pl.x < 6*30){//player hit left edge
				 ...
				 for(var i=0; i<maze.map.length; i++){
					for(var j=0; j<shiftAmount;j++){
						...
						maze.map[i].pop();
						maze.map[i].unshift(arrPieces[rand()*arrPieces.length>>>0]);
				 ...
				 for(var i=0; i<maze.map.length; i++){
					 for(var j=0; j<shiftAmount;j++){
						maingame.addGround((maze.map[0].length)*30,i*30,""+xCurr+":"+yCurr);
						rand = Alea(seed,xCurr,yCurr);
						 maze.map[i].shift();
						 maze.map[i].push(arrPieces[rand()*arrPieces.length>>>0]);
